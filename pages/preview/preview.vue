<template>
	<view class="preview" v-if="currentInfo">
		...
		<uni-popup ref="scorePopup" :safe-area="false" :is-mask-click="false">
...
<script setup>
	...
	classList.value = storgClassList.map(item => {
		return {
			...item,
			picurl: item.smallPicurl.replace("_small.webp", ".jpg") //替换为大图
		}
	})
	...
	const submitScore = async () => {
		...
		if (res.errCode === 0) {
			...
			console.log('评分后列表数据',classList,userScore.value)
	...
	function readImgsFun() {
		readImgs.value.push(
			currentIndex.value <= 0 ? classList.value.length - 1 : currentIndex.value - 1, // 判断临界值，第一张图片，预加载最后一张图
			currentIndex.value,
			currentIndex.value >= classList.value.length - 1 ? 0 : currentIndex.value + 1 //最后一张图时，预加载第一张图
		)
