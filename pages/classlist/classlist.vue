...
onReachBottom(()=>{
	if(noData.value) return; // 没有更多数据时，不再调用接口
	queryParams.pageNum++;
	getClassList();
})
...
const getClassList = async ()=>{
	...
	if(queryParams.pageSize > res.data.length) noData.value = true; //判断返回列表数据小于每页数据时，没有更多数据标记为true
	uni.setStorageSync("storgClassList",classList.value);	 // 缓存列表数据
